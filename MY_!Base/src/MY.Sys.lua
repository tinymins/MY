...
function LIB.FormatDataStructure(data, struct, assign, metaSymbol)
	if not assign then
		data = FullClone(data)
	end
	...
	local ret = {data = data}
	local aParentStack = {ret}
	local aKeyStack = {'data'}
	local aModeStack = {'value'}
	local aStructStack = {struct}
	local nStack, tParent, oKey, oValue, eMode = 1
	while nStack ~= 0 do
		-- 取出下一个待处理的
		tParent = aParentStack[nStack]
		eMode = aModeStack[nStack]
		oKey = aKeyStack[nStack]
		oValue = tParent[oKey]
		struct = aStructStack[nStack]
		data = eMode == 'key' and oKey or tParent[oKey]
		nStack = nStack - 1
		-- 标准化参数
		...
		if dataTypeExists then
			...
			if dataType == 'table' and defaultDataType == 'table' then
				for k, v in pairs(defaultData) do
					...
					nStack = nStack + 1
					aParentStack[nStack] = data
					aKeyStack[nStack] = k
					aModeStack[nStack] = 'value'
					aStructStack[nStack] = defaultData[k]
				end
			end
			...
			if dataType == 'table' and keyTemplate then
				for k, v in pairs(data) do
					if not skipKeys[k] then
						...
						aParentStack[nStack] = data
						aKeyStack[nStack] = k
						aModeStack[nStack] = 'key'
						aStructStack[nStack] = keyTemplate
					end
				end
			end
			...
			if dataType == 'table' and childTemplate then
				for k, v in pairs(data) do
					if not skipKeys[k] then
						...
						aKeyStack[nStack] = k
						aModeStack[nStack] = 'value'
						aStructStack[nStack] = childTemplate
					end
				end
			end
				...
				for i, v in pairs(data) do
					if type(i) == 'number' then
						if not skipKeys[k] then
							...
							aKeyStack[nStack] = i
							aModeStack[nStack] = 'value'
							aStructStack[nStack] = arrayTemplate
						end
					end
				end
			end
				...
				for k, v in pairs(data) do
					if type(k) ~= 'number' then
						if not skipKeys[k] then
							...
							aKeyStack[nStack] = k
							aModeStack[nStack] = 'value'
							aStructStack[nStack] = dictionaryTemplate
						end
					end
				end
			end
