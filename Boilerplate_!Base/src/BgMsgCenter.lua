...
-------------------------------------------------------------------------------------------------------
local _L = LIB.LoadLangPack(PACKET_INFO.FRAMEWORK_ROOT .. 'lang/bgmsg/')
-----------------------------------------------------------------------------------------------------------
-- 测试用（请求共享位置）
...
-- 测试用（查看版本信息）
LIB.RegisterBgMsg(NSFormatString('{$NS}_VERSION_CHECK'), function(_, oData, nChannel, dwTalkerID, szTalkerName, bSelf)
	if bSelf then
		return
	end
...
-- 测试用（调试工具）
LIB.RegisterBgMsg(NSFormatString('{$NS}_GFN_CHECK'), function(_, oData, nChannel, dwTalkerID, szTalkerName, bSelf)
	if bSelf or LIB.IsDebugClient(true) then
		return
	end
	LIB.SendBgMsg(szTalkerName, NSFormatString('{$NS}_GFN_REPLY'), {oData[1], XpCall(Get(_G, oData[2]), select(3, unpack(oData)))}, true)
end)
...
-- 搬运JH_ABOUT
LIB.RegisterBgMsg(NSFormatString('{$NS}_ABOUT'), function(_, data, nChannel, dwID, szName, bIsSelf)
	if data[1] == 'Author' then -- 版本检查 自用 可以绘制详细表格
		...
		LIB.SendBgMsg(PLAYER_TALK_CHANNEL.RAID, NSFormatString('{$NS}_ABOUT'), {'info',
...
do
	local LAST_TIME = {}
	LIB.RegisterBgMsg(NSFormatString('{$NS}_MAP_COPY_ID_REQUEST'), function(_, data, nChannel, dwTalkerID, szTalkerName, bSelf)
		if bSelf then
			--[[#DEBUG BEGIN]]
		...
		if bResponse then
			local function fnAction(tMapID)
				LIB.SendBgMsg(PLAYER_TALK_CHANNEL.RAID, NSFormatString('{$NS}_MAP_COPY_ID'), {dwMapID, tMapID[dwMapID] or -1}, true)
			end
...
do
	...
	local function OnSwitchMap(dwMapID, dwSubID, aMapCopy, dwTime)
		...
		--[[#DEBUG END]]
		LIB.SendBgMsg(PLAYER_TALK_CHANNEL.RAID, NSFormatString('{$NS}_SWITCH_MAP'), {dwMapID, dwSubID, aMapCopy, dwTime}, true)
	end
	...
	local function OnEnterMap(dwMapID, dwSubID, aMapCopy, dwTime, dwSwitchTime, nCopyIndex)
		...
		LIB.Debug(PACKET_INFO.NAME_SPACE, szDebug, DEBUG_LEVEL.LOG)
		--[[#DEBUG END]]
		LIB.SendBgMsg(PLAYER_TALK_CHANNEL.RAID, NSFormatString('{$NS}_ENTER_MAP'), {dwMapID, dwSubID, aMapCopy, dwTime, dwSwitchTime, nCopyIndex}, true)
	end
	...
	LIB.RegisterBgMsg(NSFormatString('{$NS}_ENTER_MAP_REQ'), function(_, data, nChannel, dwTalkerID, szTalkerName, bSelf)
...
do
	...
	LIB.RegisterBgMsg(NSFormatString('{$NS}_TEAMTOOLS_ACHI_REQ'), function(_, data, nChannel, dwTalkerID, szTalkerName, bSelf)
		if bSelf then
			--[[#DEBUG BEGIN]]
		...
		if bResponse then
			...
			LIB.SendBgMsg(PLAYER_TALK_CHANNEL.RAID, NSFormatString('{$NS}_TEAMTOOLS_ACHI_RES'), {aAchieveRes, aCounterRes}, true)
		end
	end)
	LIB.RegisterEvent({
		'NEW_ACHIEVEMENT',
...
LIB.RegisterBgMsg(NSFormatString('{$NS}_OUTPUT_BUFF'), function(_, data, nChannel, dwTalkerID, szTalkerName, bSelf)
	local aRes = {}
	local me = GetClientPlayer()
...
do
	local LAST_TIME = 0
	LIB.RegisterBgMsg(NSFormatString('{$NS}_GLOBAL_ID_REQUEST'), function(_, data, nChannel, dwTalkerID, szTalkerName, bSelf)
		if bSelf then
			--[[#DEBUG BEGIN]]
		...
		--[[#DEBUG END]]
		if bResponse then
			LIB.SendBgMsg(PLAYER_TALK_CHANNEL.RAID, NSFormatString('{$NS}_GLOBAL_ID'), LIB.GetClientUUID(), true)
