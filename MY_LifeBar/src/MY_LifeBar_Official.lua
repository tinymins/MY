...
if not IsLocalFileExist(X.FormatPath({'config/lifebar-official.jx3dat', X.PATH_TYPE.GLOBAL})) then
	return
end
--------------------------------------------------------------------------
...
local function SetCaptionColor(dwID, nR, nG, nB)
	...
	tColor = { nR = nR, nG = nG, nB = nB, dwColor = 255 * 16711680 + nR * 65536 + nG * 256 + nB }
