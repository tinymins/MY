...
_MY_Logoff.OnPanelActive = function(wnd)
    local ui = MY.UI(wnd)
    -- 条件登出
    local offset = { x = 50, y = 70 }
    ui:append('Label_ConditionLogoff','Text'):find('#Label_ConditionLogoff'):text(_L['# condition logoff']):pos(30,20)
    -- <hr />
    ui:append('Image_ConditionLogoff_Spliter','Image'):find('#Image_ConditionLogoff_Spliter'):pos(5,43):size(636,2):image('UI/Image/UICommon/ScienceTreeNode.UITex',62)
    --指定目标血量低于指定百分比下线
    ui:append('WndCheckBox_TargetBloodLessLogOff','WndCheckBox'):children('#WndCheckBox_TargetBloodLessLogOff'):text(_L['while']):pos(offset.x+10,offset.y+120):check(MY_Logoff.bTargetBloodLessLogOff or false):check(function(b)MY_Logoff.bTargetBloodLessLogOff=b end)
    ui:append('WndComboBox_Target', 'WndComboBox'):children('#WndComboBox_Target'):text((MY_Logoff.dwTargetBloodLessLogOff and MY_Logoff.szTargetBloodLessLogOff..'('..MY_Logoff.dwTargetBloodLessLogOff..')') or _L['[ select a target ]']):pos(offset.x+60,offset.y+120):width(290):menu(function() 
        local t = {}
    ...
    ui:append('Label_LessThan','Text'):find('#Label_LessThan'):text(_L['life below       %']):pos(offset.x+360,offset.y+120)
    ui:append('WndEditBox_TargetBloodLess','WndEditBox'):children('#WndEditBox_TargetBloodLess'):pos(offset.x+420,offset.y+123):size(30,22):text(MY_Logoff.nTargetBloodLessLogOff):change(function(txt) MY_Logoff.nTargetBloodLessLogOff = tonumber(txt) or MY_Logoff.nTargetBloodLessLogOff end)
    -- 指定玩家消失后下线
    ui:append('WndCheckBox_PlayerLeaveLogOff','WndCheckBox'):children('#WndCheckBox_PlayerLeaveLogOff'):text('当'):pos(offset.x+10,offset.y+80):check(MY_Logoff.bPlayerLeaveLogOff or false):check(function(b)MY_Logoff.bPlayerLeaveLogOff=b end)
    ui:append('WndComboBox_PlayerLeave', 'WndComboBox'):children('#WndComboBox_PlayerLeave'):text(_L('%d player(s) selected',Count(MY_Logoff.aPlayerLeaveLogOff))):pos(offset.x+60,offset.y+80):width(290):menu(function() 
        local t = {}
        for dwID, szName in pairs(MY_Logoff.aPlayerLeaveLogOff) do
            if szName and szName~='' then
                table.insert(t, {
    ...
    ui:append('Label_PlayerLeaveWhen','Text'):find('#Label_PlayerLeaveWhen'):text(_L['all disappeared']):pos(offset.x+360,offset.y+80)
    -- 自身等级到达指定值下线
    ui:append('WndCheckBox_ClientLevelOverLogOff','WndCheckBox'):children('#WndCheckBox_ClientLevelOverLogOff'):text(_L['while client level exceeds']):pos(offset.x+10,offset.y+40):check(MY_Logoff.bClientLevelOverLogOff or false):check(function(b)MY_Logoff.bClientLevelOverLogOff=b end)
    ui:append('WndEditBox_ClientLevelOverLogOff','WndEditBox'):children('#WndEditBox_ClientLevelOverLogOff'):pos(offset.x+140,offset.y+40):size(30,22):text(MY_Logoff.nClientLevelOverLogOff):change(function(txt) MY_Logoff.nClientLevelOverLogOff = tonumber(txt) or MY_Logoff.nClientLevelOverLogOff end)
    -- 指定时间后下线
    ui:append('WndCheckBox_TimeOutLogOff','WndCheckBox'):children('#WndCheckBox_TimeOutLogOff'):text(''):pos(offset.x+10,offset.y):check(MY_Logoff.bTimeOutLogOff or false):check(function(b)MY_Logoff.bTimeOutLogOff=b end)
    ui:append('WndEditBox_TimeOutLogOff','WndEditBox'):children('#WndEditBox_TimeOutLogOff'):pos(offset.x+35,offset.y):size(60,22):text(MY_Logoff.nTimeOut):change(function(txt) MY_Logoff.nTimeOut = tonumber(txt) or MY_Logoff.nTimeOut end)
    ui:append('Label_TimeOutWhen','Text'):find('#Label_TimeOutWhen'):text(_L['second(s) later']):pos(offset.x+100,offset.y-3)
    -- 符合条件时
    ui:append('Label_ReturnTo','Text'):find('#Label_ReturnTo'):text(_L['While it meets any condition below']):pos(offset.x,offset.y+155)
    ui:append('WndComboBox_ReturnTo', 'WndComboBox'):children('#WndComboBox_ReturnTo'):text(_L['return to role list']):pos(offset.x+140,offset.y+158):width(130):menu({{
    ...
    ui:append('WndButton_Print','WndButton'):children('#WndButton_Print'):text(_L['send to ...']):pos(offset.x+390,offset.y):menu({
        --SYS
    ...
    -- 快速登出
    ui:append('Label_ExpressLogoff','Text'):find('#Label_ExpressLogoff'):text(_L['# express logoff']):pos(30,300)
    -- <hr />
    ui:append('Image_ExpressLogoff_Spliter','Image'):find('#Image_ExpressLogoff_Spliter'):pos(5,323):size(636,2):image('UI/Image/UICommon/ScienceTreeNode.UITex',62)
    ui:append('WndButton_ReturnToCha','WndButton'):children('#WndButton_ReturnToCha'):text(_L['return to role list']):pos(30,350):width(120):click(function()MY_Logoff.LogOffEx(false)end)
    ui:append('WndButton_ReturnToLogin','WndButton'):children('#WndButton_ReturnToLogin'):text(_L['return to game login']):pos(155,350):width(120):click(function()MY_Logoff.LogOffEx(true)end)
    ui:append('WndButton_ReturnToChaEx','WndButton'):children('#WndButton_ReturnToChaEx'):text(_L['return to role list while not fight']):pos(280,350):width(170):click(function()MY_Logoff.LogOffEx(false,true)end)
    ui:append('WndButton_ReturnToLoginEx','WndButton'):children('#WndButton_ReturnToLoginEx'):text(_L['return to game login while not fight']):pos(450,350):width(170):click(function()MY_Logoff.LogOffEx(true,true)end)
