...
def __is_path_include(pkg_name, cwd, d):
    ...
    if (
        ...
        if os.path.basename(cwd) == pkg_name:
            ...
            return (
                "dependence=" + pkg_name
                in open(os.path.join(cwd, "package.ini"), encoding="GBK").read()
            )
        elif os.path.exists(os.path.join(cwd, "info.ini")):
            return (
                "dependence=" + pkg_name
                in open(os.path.join(cwd, "info.ini"), encoding="GBK").read()
            )
        return False
    return True
            ...
            if (
                ...
                if crc_changed:
                    ...
                    for count, line in enumerate(
                        codecs.open(filepath, "r", encoding="gbk")
                    ):
            ...
            if fileType and folderOut and fileOut:
                ...
                if crc_changed:
                    try:
                        ...
                        if (
                            fileType == "info"
                            and cwd.find(cpkg_path) == 0
                            and original_text.find("package=") == -1
                        ):
                            original_text = (
                        ...
                        if not os.path.exists(folderOut):
                            os.mkdir(folderOut)
                        with codecs.open(
                            os.path.join(folderOut, fileOut), "w", encoding="utf8"
                        ) as f:
                            f.write(original_text)
